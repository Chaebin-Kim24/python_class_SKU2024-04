...
#과제 4.26
lan=['Park(Java city)','kim(C city)','Kang(Bython city)','less(Bython city)','Hong(Ruby city)','cho(Bython city)', 'Koo(C city, Ryu(C++ city)']
...
#2)*****
animals=['dog','cat','tiger','lion']
print('animals =',animals[1],animals[2],animals[3],animals[0])
#3)
animals=['dog','cat','tiger','lion']
print('animals =',animals)
for i in range(4):
...
#과제 5.18
#1)
s_list=['abc','bcd','bcdefg','abbba','cddc','opq']
s=s_list[0]

for i in s_list[1:]:
    if len(i)<len(s):
        s=i
    elif len(i)==len(s) and i<s:
        s=i
