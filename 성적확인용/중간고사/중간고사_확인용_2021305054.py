...
##### 주관식 문제 성적 (문항당 만점: 2점) #####
문제 11. a = p6_str[i:] + p6_str[:i]          2점

문제 12. p7_src_str[3:] + p7_src_str[:3]      2점

문제 13. p7_dst_str[p7_src_str.index(a)]      2점
...
def longest(s):
    ...
    for i in range(len(s)):        
        for j in range(i+1,len(s)+1): 
            sub = s[i:j] 
            if ing(sub) and len(sub) > len(maximum):
                maximum = sub
    return maximum 
...
# 정답: p7_dst_str[p7_src_str.index(a)]
...
p9_my_str = "ATTACK"
print("가장 긴 문자열은:", longest(p9_my_str))
