...
##### 주관식 문제 성적 (문항당 만점: 2점) #####
문제 11. len(p6_str)+i                                        1점

문제 12. for i in range(0, len(p7_dst_str))                   1점
            len(p7_dst_str)+3
문제 13.
...
문제 17.                                                      1점
    ...
    len(longest_odd_palindromic_from_root('Hello my name is python',2))
    return ret, i
...
def longest_even_palindromic_from_root(my_string, index):
    ...
    len(longest_even_palindromic_from_root('Hello my name is python',2))
    return ret, i
    
문제 19.

문제 20-1.
def my_format_int(int_data, width=0):                        1점
    ret = ''

    for i in range(0, index):
        ret += my_int(my_format_int(6,0)) + ', '

    print(my_format_int(6,0))
    return ret
    
문제 20-2.
def my_format_float(float_data, width=0, precision=6):       1점
    ret = ''

    for i in range(0, index):
        ret += my_float(my_format_float(3.14,0,6)) + ', '

    print(my_format_float(3.14,0,6))
    return ret
...
for i in range(0, index_end):
    if p2_list[i] > p2_list[i + 1]:
        p2_list[i], p2_list[i + 1] = p2_list[i + 1], p2_list[i(p2_list)]
    print
...
# 정답:
# for i in range(0, len(p7_dst_str))
#     len(p7_dst_str)+3
