...
def my_format_int(int_data, width = 0):
    ret = ''
    ret = ' ' * (width - len(str(int_data))) + str(int_data)
    return ret

문제 20-2.                            2점
def my_format_float(float_data, width = 0, precision = 6):
    ret = ''
    str_list = str(float_data).split('.')

    ret = ' ' * (width - len(str(float_data)) - precision + 1) + str_list[0] + '.' + str_list[1] + (precision - len(str_list[1])) * '0'
    return ret
...
def my_format_float(float_data, width=0, precision=6):
    ...
    ret = ' ' * (width - len(str(float_data)) - precision + 1) + str_list[0] + '.' + str_list[1] + (
            precision - len(str_list[1])) * '0'
    return ret

print(my_format_float(12345.678910, 5, 2))