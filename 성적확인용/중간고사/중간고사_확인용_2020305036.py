...
##### 주관식 문제 성적 (문항당 만점: 2점) #####
문제 11. print(p6_str[i + 1:] + p6_str[:i + 1])       2점

문제 12. [3:] + p7_src_str[:3]                        2점

문제 13. [p7_src_str.index(a) - 3]                    2점

문제 14. encoded_str = ''                             2점

문제 15.                                              2점
def ciper(a):
    return p8_dst_str[p8_src_str.index(a) + 3]
...
def longest_odd_palindromic_from_root(my_string, index):
    i = 1
    ret = my_string[index]
    while (index - i >= 0) & (index + i < len(my_string) - 1) & (my_string[index + i] == my_string[index - i]):
...
def longest_odd_palindromic_from_root(my_string, index):
    i = 0
    ret = ''
    while (index - i >= 0) and (index + 1 + i < len(my_string) - 1) and (my_string[index + 1 + i] == my_string[index - i]):
        ret = my_string[index - i] + ret + my_string[index - i]
        i += 1
    i = len(ret)
    return ret, i
...
문제 19.                                              2점
maxS = ('',0)
for i in range(len(p9_my_str)):
    tmp = longest_odd_palindromic_from_root(p9_my_str, i)
    if tmp[1] > maxS[1]:
        maxS = tmp
...
def my_format_int(int_data, width = 0):
    ...
    if  len(str_data) <= width:
        ...
        ret = str_data[len(str_data) - width : ]
...
def my_format_float(float_data, width = 0, precision = 6):
    ...
    if  len(str_data) <= width + precision + 1:
        ret = str_data
    else:
        field, prec = str_data.split('.')
        if len(field) <= width:
            ret = field
        else :
            ret = field[len(field) - width : ]
        if len(prec) <= precision:
            ret += prec
        else:
            ret += '.' + prec[:precision]
...
def ciper(a):
    return p7_dst_str  [p7_src_str.index(a) - 3] # 주석 처리된 부분
