...
문제 12. p7_src_str[3:] + p7_src_str[:3]      2점
...
def ciper(a):
    if a in p8_src_str:
        index = p8_src_str.index(a)
        return p8_dst_str[index]
    else:
        return a
...
def longest_odd_palindromic_from_root(my_string, index):
    ...
    while start_index >= 0 and end_index < length and my_string[start_index] == my_string[end_index]:
        current_length = end_index - start_index + 1
        if current_length > max_length:
            max_length = current_length
            ret = my_string[start_index:end_index + 1]
        start_index -= 1
        end_index += 1
...
def my_format_float(float_data, width=0, precision=6):
    ...
    ret = format_string.format(float_data)
    if width > 0:
        ret = ret.rjust(width)
    return ret
...
print("longest palindromic string:", longest_palindrome)
