...
문제 17.                                                      1점
    my_string=remove_space(my_string)
    i = 1    
    ret = ''
    left_string=my_string[:index:]
    right_string=my_string[index::-1]
    if left_string>=my_string:
        for j in range(len(right_string)):
            if left_string[j]==right_string[j]:
                i+=1
                ret+=left_string[j]
            else:
                break
    else:
        for j in range(len(left_string)):
            if left_string[j]==right_string[j]:
                i+=1
                ret+=left_string[j]
            else:
...
def my_format_int(int_data, width = 0):
    ret = str(int_data)+str(width)
    return ret

문제 20-2.                                                    1점
def my_format_float(float_data, width = 0, precision = 6):
    ret = str(float_data)+str(width)+str(precision)
    return ret
...
def ciper(a):
    return '' #p7_dst_str.index(a)  # 주석 처리된 부분
...
print(longest_odd_palindromic_from_root('abaabcbadddd',5))
# 문제 18.
...
def longest_even_palindromic_from_root(my_string, index):
    ...
    return ret, i
print(longest_even_palindromic_from_root('abbadddabccbaa',1))
