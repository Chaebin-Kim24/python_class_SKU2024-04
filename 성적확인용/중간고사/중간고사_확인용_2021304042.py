...
##### 주관식 문제 성적 (문항당 만점: 2점) #####
문제 11. print(p6_str[i:] + p6_str[:i])                   2점

문제 12. p7_src_str = "ABCDEFGHIJKLMNOPQURSTUVWXYZ"       2점
        p7_dst_str = p7_src_str[3:] + p7_src_str[:3]
        print(p7_dst_str)

문제 13. return p7_dst_str[p7_src_str.index(a)]           2점

문제 14. encoded_str = ''                                 2점

문제 15. def ciper(a):                                    2점
            if a in p8_src_str:
                return p8_dst_str[p8_src_str.index(a)]
            else:
                return a
...
문제 16. def remove_space(my_string):                     2점
            ret = ''
...
문제 17. def longest_odd_palindromic_from_root(my_string, index):     2점
            i = 1
            ret = my_string[index]
            while index - i >= 0 and index + i < len(my_string) and my_string[index - i] == my_string[index + i]:
                ret = my_string[index - i] + ret + my_string[index + i]
                i += 1
            return ret, i

문제 18. def longest_even_palindromic_from_root(my_string, index):    2점
            i = 1
            while index - i >= 0 and index + i + 1 < len(my_string):
                if my_string[index - i] != my_string[index + i + 1]:
                    break
                i += 1
            ret = my_string[index - i + 1:index + i + 1]
            return ret, 2 * i
...
문제 19. def find_longest_palindrome(my_string):                      3점
            longest = ''
   
        def expand_from_center(left, right):
            while left >= 0 and right < len(my_string) and my_string[left] == my_string[right]:
                left -= 1
                right += 1
            return my_string[left + 1:right]
        ...
        p9_my_str = "ATTACK"
        longest_palindrome = find_longest_palindrome(p9_my_str)
        print(longest_palindrome)  

문제 20-1. def my_format_int(int_data, width = 0):                    3점
              return '{0:>{width}}'.format(int_data, width=width)

문제 20-2. def my_format_float(float_data, width = 0, precision = 6): 3점
              return '{0:>{width}.{precision}f}'.format(float_data, width=width, precision=precision)
...
# 정답: p7_src_str = "ABCDEFGHIJKLMNOPQURSTUVWXYZ"
# p7_dst_str = p7_src_str[3:] + p7_src_str[:3]
# print(p7_dst_str)
...
def ciper(a):
    return p7_dst_str[p7_src_str.index(a)]  # 주석 처리된 부분
...
# 정답: return p7_dst_str[p7_src_str.index(a)]
