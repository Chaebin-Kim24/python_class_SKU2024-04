...
##### 주관식 문제 성적 (문항당 만점: 2점) #####
문제 11. rotated_str = p6_str[i:] + p6_str[:i]        2점
        print(rotated_str)

문제 12. rotated_str = p6_str[i:] + p6_str[:i]        1점
        print(rotated_str)

문제 13. p7_dst_str[src_str.index(a)]                 1점

문제 14. encoded_str = "  "                           1점
...
for i in range(0, len(p6_str)):
    _ = i
    rotated_str = p6_str[i:] + p6_str[:i]# 코드를 입력할 위치
    print(rotated_str)
#정답: rotated_str = p6_str[i:] + p6_str[:i]
#print(rotated_str)
...
def ciper(a):
    return ' '# p7_dst_str[src_str.index(a)]  # 주석 처리된 부분
