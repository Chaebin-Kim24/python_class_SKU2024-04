...
문제 12. p7_dst_str = p7_src_str[3:] + p7_src_str[:3]         2점

문제 13. p7_dst_str.append(p7_src_str.index(a))               2점
...
문제 17.
def longest_odd_palindromic_from_root(my_string, index):    2점
    max_length = 1
    palindromic_string = my_string[index]


    l, r = index - 1, index + 1
    while l >= 0 and r < len(my_string) and my_string[l] == my_string[r]:
        if r - l + 1 > max_length:
            max_length = r - l + 1
            palindromic_string = my_string[l:r+1]
        l -= 1
        r += 1

    return palindromic_string, max_length
...
def my_format_int(int_data, width=0):
    ...
    if width > 0:
        if len(data_str) < width:
            ret = ' ' * (width - len(data_str)) + data_str
        else:
            ret = data_str[:width]
    else:
        ret = data_str
...
# 정답: p7_dst_str = p7_src_str[3:] + p7_src_str[:3]
...
# 정답: p7_dst_str.append(p7_src_str.index(a))
...
def p9_my_str(longest palindromic
