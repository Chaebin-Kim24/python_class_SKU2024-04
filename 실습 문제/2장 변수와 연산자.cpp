...
#define _CRT_SECURE_NO_WARNINGS_ /* scanf */
...
int main()
{
    ...
    * name_total = "Total";
    ...
    std::cout << "please type any characters: ";
    std::string input_str;
    std::cin >> input_str;
    std::cout << " input string: " << input_str << std::endl; // C++
    
    char input_str2[100];
    printf("please type any characters: ");
    scanf("%99s", input_str2);
    printf(" input string: %s\n", input_str2); // C
    
    
    // 심화학습 : 파이썬 input 함수를 C/C++로 구현해보기
    
#define input_c(...) input_with_message(__VA_ARGS__ __VA_OPT__(,) "")

#define input_with_message( MESSAGE, ...) printf_malloc_scanf( MESSAGE )
    
    char* printf_malloc_scanf(const char* msg);
    
    char* input_c_str = input_c("Test input C: ");
    printf("%s ...ok\n", input_c_str);
    free(input_c_str);                      // input C 테스트
    

    auto input_cpp = [](const char* msg) {
        std::cout << msg;
        std::string input_str;
        std::cin >> input_str;
        return input_str;
    ...
    std::cout << input_cpp("Test input C++: ") << " ...ok" << std::endl; // input C++ 테스트

    // 심화학습 : 파이썬 print 함수를 C/C++로 구현해보기

    const char* sep = " ";
    const char* end = "\n";

    
#define print_c(...) print_max8(__VA_ARGS__ __VA_OPT__(,) 8, 7, 6, 5, 4, 3, 2, 1, 0);
...
#define print_1(X) \
    printf( \
_Generic( (X), int:"%d", double:"%g", char*:"%s", const char*:"%s", char:"%c", bool:(X)?"true":"false") \
      , X)

    print_c("Hello, print_c", '!', 1, 2.3, true); // 테스트 print C


    auto print_cpp = [sep, end](auto... inputs) {
        int _[] = { (std::cout << inputs << sep, 0)... };
        std::cout << end;
    };

    print_cpp("Test print_cpp:", 1, 'g', 0.0, true, "...ok"); // 테스트 print C++
...
char* printf_malloc_scanf(const char* msg){
    ...
    snprintf(format_string, 9, "%%%ds", size-1);
